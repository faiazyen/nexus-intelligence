...
scheduled. Classification hits OpenRouter's free tier first, an
ultra-cheap paid tier second, and the rule-based heuristic as a
guaranteed final fallback — see app.core.llm_router.
...
from app.core.llm_router import BudgetExceededError, cost_tracker
from app.services.audit import log_pipeline_event
...
async def run_pipeline(org_id: uuid.UUID, db: AsyncSession) -> dict:
    ...
    try:
        ...
        await log_pipeline_event(db, org_id, "collector_error", {"stage": "signal_scout", "error": str(exc)})

    # Stage 2 — Intent Classifier (enforce budget first).
    try:
        ...
        await log_pipeline_event(db, org_id, "budget_exceeded", {"stage": "intent_classifier", "error": str(exc)})
    except Exception as exc:  # noqa: BLE001
        logger.exception("Supervisor: intent classifier failed")
        summary["human_review"].append({"stage": "intent_classifier", "error": str(exc)})
        await log_pipeline_event(db, org_id, "collector_error", {"stage": "intent_classifier", "error": str(exc)})
    ...
    try:
        ...
        await log_pipeline_event(db, org_id, "collector_error", {"stage": "account_scorer", "error": str(exc)})
